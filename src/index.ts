...
export default class CanvasToReact extends EventBus {
  ...
  constructor(el: HTMLCanvasElement | string, imgSrc?: string) {
    ...
    if (
      Object.prototype.toString.call(container).includes('HTMLCanvasElement')
    ) {
  ...
  initScreen() {
    ...
    this.canvas.addEventListener('mousedown', (e: MouseEvent) => {
      ...
      if (e.buttons === 2) {
        ...
        this.ctrlIndex = ctrls.findIndex((coor: Point) =>
          this.isPointInCircle(mousePoint, coor, this.ctrlRadius)
        );
        if (this.ctrlIndex > -1) {
        ...
        // 是否正在创建多边形
        const oncreating =
          this.activeShape?.type === 2 && this.activeShape.creating;
        if (oncreating) {
          // 正在创建多边形
          // 多边形新增点
    ...
    this.canvas.addEventListener('mousemove', (e: MouseEvent) => {
      ...
      if (e.buttons === 2 && e.which === 3) {
        ...
        if (this.ctrlIndex > -1 && this.isInBackground(e)) {
          ...
          if (this.activeShape.type === 1) {
            ...
            if (a1 - a0 >= this.MIN_WIDTH && b1 - b0 >= this.MIN_HEIGHT) {
              ...
              this.emit(
                'error',
                `宽不能小于${this.MIN_WIDTH},高不能小于${this.MIN_HEIGHT}。`
              );
            }
          ...
          this.activeShape.type === 1 &&
          this.isInBackground(e)
        ) {
          // 创建矩形
        ...
        this.activeShape?.type === 2 &&
        (this.activeShape as Polygon)?.creating
      ) {
        // 多边形添加点
        this.update();
      }
    ...
    document.body.addEventListener('keyup', (e: KeyboardEvent) => {
      if (this.lock) return;
      if (['Escape', 'Backspace','Delete'].includes(e.key) && this.activeShape) {
  ...
  isInBackground(e: MouseEvent): boolean {
    ...
    return (
      offsetX >= this.originX / this.scale &&
      offsetY >= this.originY / this.scale &&
      offsetX <= this.originX / this.scale + this.IMAGE_ORIGIN_WIDTH &&
      offsetY <= this.originY / this.scale + this.IMAGE_ORIGIN_HEIGHT
    );
  }
  /**
   * 判断是否在矩形内
  ...
  isPointInPolygon(point: Point, coor: Point[]): boolean {
    ...
    const areaData = this.offlineCtx.getImageData(
      0,
      0,
      this.WIDTH,
      this.HEIGHT
  ...
  isPointInCircle(point: Point, center: Point, r: number): boolean {
    ...
    const distance = Math.sqrt(
      (x0 - (x - this.originX)) ** 2 + (y0 - (y - this.originY)) ** 2
    );
    return distance <= r;
  }
  /**
  ...
  drawRect(shape: Rect) {
    ...
    this.ctx.restore();
    const labelCoor: Point = [
      coor[0][0] + (coor[1][0] - coor[0][0]) / 2,
      coor[0][1] + (coor[1][1] - coor[0][1]) / 2,
    ];
   ...
   */
  drawDot(shape: Dot) {
    const { labelFillStyle, strokeStyle, fillStyle, active, coor, label } =
      shape;
    ...
    const labelCoor = [
      coor[0],
      coor[1] + ((this.pointRadius + 2) * 1) / this.scale,
    ];
    ...
    if (label.length) {
      const newStr =
        label.length < this.labelMaxLen + 1
          ? label
          : `${label.substr(0, this.labelMaxLen)}...`;
      ...
      this.ctx.fillRect(
        ...
        y,
        text.width + 8,
        this.labelFontSize * 1.3
      );
      this.ctx.fillStyle = this.labelFontColor;
      this.ctx.fillText(
        newStr,
        x - (text.width + 4) / 2,
        y + this.labelFontSize
      );
      this.ctx.restore();
    }
  }
  ...
  update() {
    ...
    this.dataset.forEach((shape) => {
      switch (shape.type) {
        case 1:
          shape.label = shape.label ? shape.label : this.currentLabel;
  ...
  setScale(type: boolean, e?: MouseEvent) {
    ...
    if (this.zoomCenter === 'mouse' && e) {
      ...
      const newOffsetX =
        ratio * (e.clientX - containerPosition.pageLeft - this.originX);
      const newOffsetY =
        ratio * (e.clientY - containerPosition.pageTop - this.originY);

      this.originX =
        this.originX +
        (e.clientX - containerPosition.pageLeft - this.originX) -
        newOffsetX;
      this.originY =
        this.originY +
        (e.clientY - containerPosition.pageTop - this.originY) -
        newOffsetY;
    } else {
      // 以画布为中心，进行缩放
